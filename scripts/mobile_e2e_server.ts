...
import { runFinanceExportJob } from "../src/api/finance-export-worker.js";
...
const accountMainId = "31000000-0000-0000-0000-0000000000a1";
const accountCashId = "32000000-0000-0000-0000-0000000000a1";
...
const currentPeriodStart = `${currentYear}-${String(currentMonth + 1).padStart(2, "0")}-01`;
const currentPeriodEnd = `${currentYear}-${String(currentMonth + 1).padStart(2, "0")}-${String(new Date(currentYear, currentMonth + 1, 0).getDate()).padStart(2, "0")}`;
const currentTxDay = String(Math.min(now.getDate(), 28)).padStart(2, "0");
const currentTransactionAt = new Date(currentYear, currentMonth, Number(currentTxDay), 12, 0, 0).toISOString();
...
await db.query("INSERT INTO financial_account (id, household_id, name, account_type, currency, opening_balance, status) VALUES ($1, $2, '招商银行', 'bank', 'CNY', '10000.00', 'active')", [accountMainId, householdId]);
await db.query("INSERT INTO financial_account (id, household_id, name, account_type, currency, opening_balance, status) VALUES ($1, $2, '家庭现金', 'cash', 'CNY', '2000.00', 'active')", [accountCashId, householdId]);
...
await db.query("UPDATE financial_source SET account_id = $1 WHERE household_id = $2 AND id = $3", [accountMainId, householdId, "30000000-0000-0000-0000-0000000000a1"]);
...
await db.query("INSERT INTO budget_period (id, household_id, budget_id, period_start, period_end, amount) VALUES ($1, $2, $3, $4, $5, '3000.00')", [budgetPeriodId, householdId, budgetId, currentPeriodStart, currentPeriodEnd]);
await db.query("INSERT INTO ledger_transaction (id, household_id, occurred_at, direction, amount, currency, merchant, category, created_by) VALUES ($1, $2, $3, 'expense', '128.00', 'CNY', '家庭餐饮', '餐饮', $4)", ["70000000-0000-0000-0000-0000000000a1", householdId, currentTransactionAt, userId]);
...
await db.query("INSERT INTO asset_event (id, household_id, asset_id, occurred_at, event_type, amount, recovery_amount) VALUES ($1, $2, $3, $4, 'purchase', '3000.00', '0.00')", ["81000000-0000-0000-0000-0000000000a1", householdId, "80000000-0000-0000-0000-0000000000a1", currentTransactionAt]);
...
const importParser = new LocalFinanceImportParser(importStore);
const passwordResetDelivery = new MobileE2ePasswordResetDelivery();
const app = buildServer({ authStore, passwordResetDelivery, financeFactory: (requestScope) => new SqlFinanceRepository(pool, requestScope, importStore), familyFactory: (requestScope) => new SqlFamilyRepository(pool, requestScope), importObjectStore: importStore, exportRunner: (scope, jobId) => runFinanceExportJob(pool, scope, importStore, jobId), importRunner: (scope, batchId, jobId) => runFinanceImportJob(pool, importParser, importStore, scope, jobId) });
...
app.get<{ Querystring: { id?: string } }>("/__e2e/expire-finance-export", async (request, reply) => {
  const exportId = request.query.id;
  if (!exportId) return reply.code(400).send({ code: "EXPORT_ID_REQUIRED" });
  await db.query("RESET ROLE");
  await db.query("UPDATE finance_export_job SET download_expires_at = now() - interval '1 minute', updated_at = now() WHERE id = $1", [exportId]);
