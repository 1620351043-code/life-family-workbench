...
import { financeApi, type FinanceImportBatch, type FinanceImportErrorRow, type FinanceImportSource, type FinanceReconciliationCandidate } from "../api";
...
export function FinanceImportWizard({ onClose, onError, onOpenImportedTransactions }: { onClose: () => void; onError: (message: string) => void; onOpenImportedTransactions: (batchId: string) => void }) {
  ...
  const createBatch = () => {
    ...
    void run(async () => {
      const created = await financeApi.createImportBatch({ source_type: source, file_name: file.name, file_size: file.size, file_sha256: fileHash, object_key: `pending-upload/${fileHash}/${encodeURIComponent(file.name)}` });
      ...
      setBatch(parsed);
      setHeaderRow(parsed.detected_header_row ?? 1);
      setDataStartRow((parsed.detected_header_row ?? 1) + 1);
      setSheetName(parsed.detected_sheet ?? "Sheet1");
      setMapping(parsed.field_mapping ?? mapping);
      setStep(parsed.status === "header_detected" ? "header" : "batch");
    });
  };
  ...
  return <FinanceSheet className="finance-import-sheet" ariaLabel="账单导入向导" onClose={onClose}><div className="sheet-handle" /><div className="sheet-title"><div><span className="eyebrow">财务 · 账单导入</span><h2>{step === "commit" ? "导入已完成" : "把多份账单整理成一笔"}</h2></div><button type="button" aria-label="关闭账单导入" onClick={onClose}>×</button></div>
    ...
    {step === "select" && <section className="import-panel"><p className="import-lead">选择一份导出的账单。上传后会直接展示行号和原始内容，点击任意行即可确认表头。</p><div className="import-source-grid">{(Object.keys(sourceLabels) as FinanceImportSource[]).map((item) => <button type="button" key={item} className={source === item ? "selected" : ""} onClick={() => setSource(item)}>{sourceLabels[item]}</button>)}</div><label className="file-drop"><input type="file" accept=".csv,.xls,.xlsx,.txt,.pdf,.zip" onChange={(event) => void selectFile(event.target.files?.[0] ?? null)} /><span>＋</span><strong>{file ? file.name : "选择 CSV / XLS / XLSX / TXT / PDF / ZIP 文件"}</strong><small>{file ? `${formatBytes(file.size)} · 正在生成文件摘要` : "原始文件按家庭隔离，保留一年"}</small></label>{file && <div className="import-file-meta"><span>{sourceLabels[source]}</span><span>{fileHash ? "SHA-256 已生成" : "正在读取…"}</span></div>}<button type="button" className="primary-button wide" disabled={!file || !fileHash || busy} onClick={createBatch}>{busy ? "正在上传并解析…" : "上传并登记导入批次"}</button><button type="button" className="secondary-button wide" onClick={loadHistory}>恢复历史批次</button><small className="import-honesty">文件会先上传到当前家庭的隔离存储并校验摘要，再由解析 worker 识别表头并写入待审核来源记录。</small></section>}
    {step === "batch" && batch && <section className="import-panel"><ImportStatusCard batch={batch} file={file} /><div className="import-warning"><strong>文件已安全上传，等待解析</strong><p>原始文件已完成大小和 SHA-256 校验，但还没有把内容写入正式账本；点击继续会重新调用解析 worker，不会重复入账。</p></div><button type="button" className="primary-button wide" disabled={busy} onClick={resumeParse}>{busy ? "正在解析…" : "继续识别表头"}</button></section>}
    ...
    {step === "commit" && batch && <section className="import-panel"><ImportStatusCard batch={batch} file={file} />{summary ? <div className="commit-summary"><SummaryMetric label="新增统一账本" value={summary.inserted_transactions} onClick={() => onOpenImportedTransactions(batch.id)} /><SummaryMetric label="保留关联来源" value={summary.linked_records} onClick={() => onOpenImportedTransactions(batch.id)} /><SummaryMetric label="待处理记录" value={summary.pending_records} onClick={reopenReview} /><SummaryMetric label="失败/问题行" value={summary.failed_records} onClick={openErrors} /></div> : batch.status === "committed" ? <div className="import-success-card"><strong>这个批次已经提交到统一账本</strong><p>可以从历史批次重新打开，或返回财务首页查看对应流水。</p><button type="button" className="secondary-button wide" onClick={() => onOpenImportedTransactions(batch.id)}>查看该批次流水</button></div> : <><div className="import-warning"><strong>提交前最后确认</strong><p>已确认的重复关系只会生成一笔统一账本交易；银行优先作为锚点，支付平台保留为详细来源。</p></div><button type="button" className="primary-button wide" disabled={!canCommit || busy} onClick={commit}>{busy ? "正在写入…" : "确认写入统一账本"}</button></>}<button type="button" className="secondary-button wide" onClick={onClose}>{summary || batch.status === "committed" ? "返回财务首页" : "取消"}</button></section>}
