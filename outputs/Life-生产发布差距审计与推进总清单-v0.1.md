...
| I-010 | 请求日志和错误聚合 | `PARTIAL / REPO_READY` | 全部响应含 `x-request-id`/`trace_id`，错误响应稳定错误码；结构化日志不落请求体/Cookie/令牌/查询参数，IP、用户、家庭和邮箱哈希脱敏；journald 检索脚本与部署说明已入库。真实 staging 日志检索与服务重启待 live 验收 |
